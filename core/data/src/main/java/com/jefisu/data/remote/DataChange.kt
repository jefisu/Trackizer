...
sealed class DataChange {
    ...
    @Serializable
    data class Added(val document: FirestoreDocumentSync) : DataChange()

    @Serializable
    data class Modified(val document: FirestoreDocumentSync) : DataChange()

    @Serializable
    data class Removed(val document: FirestoreDocumentSync) : DataChange()

    inline fun onAddedOrModified(action: (FirestoreDocumentSync) -> Unit): DataChange {
